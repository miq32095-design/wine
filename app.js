...
const sampleDrinks = [
  {name:"会议结束后的自由", subtitle:"献给关掉摄像头之后的第一口空气。", emotions:["疲惫","开心"], flavors:["柠檬","气泡","晚风"], c1:"#f0a45b", c2:"#876fd5"},
  {name:"已读不回莫吉托", subtitle:"消息留在屏幕里，薄荷先替你降温。", emotions:["焦虑","烦躁"], flavors:["薄荷","青柠","微苦"], c1:"#7bd2ae", c2:"#d7c86a"},
  {name:"DDL 前夜浓缩", subtitle:"黑咖啡般的清醒，以及尚未保存的勇气。", emotions:["疲惫","焦虑"], flavors:["冷萃","可可","海盐"], c1:"#4b302e", c2:"#9a6350"},
  {name:"周五下班第一口空气", subtitle:"气泡很满，待办事项已经沉底。", emotions:["开心","期待"], flavors:["青提","气泡","橙花"], c1:"#79d2b8", c2:"#f6b85f"},
  {name:"暧昧期气泡水", subtitle:"甜度不明，气泡很多，答案暂时保密。", emotions:["心动","期待"], flavors:["覆盆子","玫瑰","气泡"], c1:"#ee7fa1", c2:"#ad83df"},
  {name:"社交电量不足冰茶", subtitle:"适合静音饮用，不建议同时回复八条消息。", emotions:["疲惫","孤独"], flavors:["乌龙","冰块","晚风"], c1:"#6b5543", c2:"#6e739d"}
];
...
function drinkVisual(drink, size="large") {
  ...
  return `
    <div class="drink-wrap ${size}" style="--c1:${drink.c1};--c2:${drink.c2};--garnish:${drink.garnishColor || "#ffb45c"}">
  ...
  const pages = [
    {
      ...
      visual: drinkVisual({c1:"#f37c9b", c2:"#a88cff", garnishColor:"#ffb45c"})
    },
    {
      title:"每一种情绪，都是一种风味。",
  ...
  return `
    ...
    <section class="hero">
      ...
      <div class="hero-drink">
        ...
        ${drinkVisual({c1:"#ef7fa2",c2:"#806cc8",garnishColor:"#f2b356"})}
      </div>
...
function drinkCard(d) {
  return `<article class="card drink-card" data-open-drink="${d.id}">
    <div class="drink-preview"><div class="tiny-glass" style="--c1:${d.c1};--c2:${d.c2}"></div></div>
...
function resetForm() {
  state.mixStep = 1;
  state.form = { emotions:[], customMood:"", intensity:58, contexts:[], note:"", energy:50, weather:"晚风", desired:"放松一点" };
}
...
function stepFooter(enabled) {
  return `<div class="step-footer"><button class="primary-btn" id="nextStep" ${enabled?"":"disabled"}>继续</button></div>`;
}
...
function bindMixEvents() {
  ...
  document.querySelectorAll("[data-mood]").forEach(btn => btn.onclick = () => {
    ...
    state.form.emotions = state.form.emotions.includes(id) ? state.form.emotions.filter(x=>x!==id) : [...state.form.emotions,id].slice(0,3);
    render();
  });
  document.querySelector("#customMood")?.addEventListener("input", e=>state.form.customMood=e.target.value);
  ...
  document.querySelectorAll("[data-energy]").forEach(btn=>btn.onclick=()=>{state.form.energy=Number(btn.dataset.energy);render();});
  document.querySelectorAll("[data-weather]").forEach(btn=>btn.onclick=()=>{state.form.weather=btn.dataset.weather;render();});
  document.querySelectorAll("[data-desired]").forEach(btn=>btn.onclick=()=>{state.form.desired=btn.dataset.desired;render();});
    ...
    <main class="app-shell no-nav mixing-screen">
      <section>
        <div class="mixing-wrap">
          ...
          ${drinkVisual({c1:m.c1,c2:m.c2,garnishColor:"#ffb45c"})}
...
function generateDrink() {
  ...
  let total=weights.reduce((s,x)=>s+x.value,0);
  weights[0].value+=100-total;
  if(state.form.customMood.trim()) {
    weights[0].value=Math.max(20,weights[0].value-10);
    weights.push({label:state.form.customMood.trim().slice(0,10),value:10});
  }
  ...
  return {
    ...
    metrics:{
      ...
      bubble,
      aftertaste,
      setting:state.form.energy<=30?"戴上耳机，独自慢慢喝":state.form.desired==="被安慰"?"盖好被子，在暖光里喝":"在晚风经过的时候慢慢喝"
    },
    c1:selected[0].c1,
...
function updateSharePreview(drink,template) {
  ...
  const palettes={
    ...
    minimal:["#eee8df","#d8d0c7","#1c1720"],
    dreamy:["#2b1740","#8d4f79","#fff8f2"]
  };
  ...
  ctx.fillStyle=text;ctx.globalAlpha=.72;ctx.font="500 30px sans-serif";ctx.letterSpacing="8px";ctx.fillText("MOOD MIXER · 今日特调",86,110);ctx.globalAlpha=1;
  // glass
  ctx.save();ctx.translate(540,650);
  ctx.strokeStyle="rgba(255,255,255,.5)";ctx.lineWidth=8;roundRect(ctx,-190,-270,380,520,48);ctx.stroke();
